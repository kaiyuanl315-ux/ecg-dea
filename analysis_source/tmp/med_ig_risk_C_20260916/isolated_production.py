...
def main():
    ...
    assert len(x)==1536
    pred=np.load(BASE/'production_predictions_mps.npy')
    gate=json.loads((BASE/'production_prediction_gate.json').read_text())
    info=gate['members'];ens=pred.mean(1)
    assert pred.shape==(1536,10) and np.isfinite(pred).all()
    assert gate['ensemble']==q.compare(ens,a['expected_mean'])
    assert gate['ensemble']['outside_prior_tolerance']==0 and not any(gate['threshold_changes'].values())
    assert q.digest(BASE/'inputs.npz')==json.loads((BASE/'production_plan_private.json').read_text())['input_sha256']
    for k in range(10):
        assert q.digest(q.WEIGHTS/f'run_{k:02d}_best_by_val_auprc.ckpt')==info[k]['sha256']
...
if __name__=='__main__':
    ...
    def isolated_integrate(device,steps,kind,subset,tag,batch):
        assert kind=='zero' and subset.startswith('indices:')
        for k in range(10):
            subprocess.run([sys.executable,str(BASE/'member_worker.py'),'--run',str(k),'--device',device,
                '--steps',str(steps),'--tag',tag,'--indices',subset.split(':',1)[1],'--batch',str(batch)],check=True)
        return original_integrate(device,steps,kind,subset,tag,batch)
    q.integrate=isolated_integrate
